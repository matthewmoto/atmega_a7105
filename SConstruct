...
def getBoardConfValues(board_conf, prefix):
  vals = {}
  for k,v in board_conf.iteritems():
    if k.find(prefix) == 0:
      vals[k] = v
  return vals

def getBoardConfValue(board_conf, board_name, processor_name, key, default=None):

  real_key = "%s.%s" % (board_name,key)
  processor_specific_key = "%s.menu.cpu.%s.%s" % (board_name,processor_name,key)

  #Try to get the processor-specific key before falling bakc to just the board name
  if processor_name != "":
    vals = getBoardConfValues(board_conf,processor_specific_key) 
    if len(vals):
      return vals.values()[0]

  vals = getBoardConfValues(board_conf,real_key)
  if len(vals):
    return vals.values()[0]
  return default
...
ARDUINO_BOARD   = resolve_var('ARDUINO_BOARD', 'pro')
ARDUINO_PROCESSOR   = resolve_var('ARDUINO_PROCESSOR', 'atmega328')
...
def find_in_tree(filename, dir_to_walk):
  ret = []
...
def get_board_names(board_files):
  '''
    returns a dictionary of board_name -> board_file for lookup
  '''
...
arduino_boards = find_in_tree("boards.txt",path.join(ARDUINO_HOME,'hardware'))
custom_boards = find_in_tree("boards.txt",path.join(SKETCHBOOK_HOME,'hardware'))
board_files  = arduino_boards + custom_boards
...
for bf in board_files:
    for line in open(bf):
        ...
        if result:
            boards[result.group(1)] = (result.group(2), bf)
'''
boards = get_board_names(board_files)
...
ARDUINO_CONF_VALUES = parseBoardFile(ARDUINO_CONF)

#make sure our processor option is available
if getBoardConfValue(ARDUINO_CONF_VALUES,ARDUINO_BOARD,ARDUINO_PROCESSOR,"build.mcu") == None:
  print "ERROR! the given processor, '%s' is not supported board list: " % (ARDUINO_BOARD)
  sys.exit(-1)

ARDUINO_CORE = path.join(path.dirname(ARDUINO_CONF),
                         'cores/', getBoardConfValue(ARDUINO_CONF_VALUES,ARDUINO_BOARD,ARDUINO_PROCESSOR,'build.core', 'arduino'))
...
#HACK HACK: Use the bundled avrdude conf all the time, the aruduino-1.6.1
#           avrdude looks in some developers home directory by default WTF!?!?!
if True or platform == 'darwin' or platform == 'win32':
    AVRDUDE_CONF = path.join(ARDUINO_HOME, 'hardware/tools/avr/etc/avrdude.conf')

AVR_BIN_PREFIX = path.join(AVR_HOME, 'avr-')

#HACK: hardcoded "avr" in this library path
ARDUINO_LIBS = [path.join(ARDUINO_HOME, 'libraries'),path.join(ARDUINO_HOME,'hardware/arduino/avr/libraries')]
if EXTRA_LIB:
...
MCU_key = "%s.menu.cpu.%s.build.mcu" % (ARDUINO_BOARD,ARDUINO_PROCESSOR)
F_CPU_key = "%s.menu.cpu.%s.build.f_cpu" % (ARDUINO_BOARD,ARDUINO_PROCESSOR)
 
MCU = ARGUMENTS.get('MCU', getBoardConfValue(ARDUINO_CONF_VALUES,ARDUINO_BOARD,ARDUINO_PROCESSOR,'build.mcu'))
F_CPU = ARGUMENTS.get('F_CPU', getBoardConfValue(ARDUINO_CONF_VALUES,ARDUINO_BOARD,ARDUINO_PROCESSOR,'build.f_cpu'))
...
#HACK: allow for non-avr variant
hwVariant = path.join(ARDUINO_HOME, 'hardware/arduino/avr/variants',
                     getBoardConfValue(ARDUINO_CONF_VALUES,ARDUINO_BOARD,ARDUINO_PROCESSOR,"build.variant", ""))
if hwVariant:
...
def fnPrintInfo(target, source, env):
    ...
    max_upload_size = getBoardConfValue(ARDUINO_CONF_VALUES,
                                        ARDUINO_BOARD,
                                        ARDUINO_PROCESSOR,
                                        "upload.maximum_size",
                                        "???")
    print "* maximum size for hex file: %s bytes" % max_upload_size
...
# Upload
UPLOAD_PROTOCOL = getBoardConfValue(ARDUINO_CONF_VALUES,ARDUINO_BOARD,ARDUINO_PROCESSOR,'upload.protocol')
UPLOAD_SPEED = getBoardConfValue(ARDUINO_CONF_VALUES,ARDUINO_BOARD,ARDUINO_PROCESSOR,'upload.speed')
...
#MM: Commented this out since it was using the avrdude prefix for all the other avr tools...
#if AVR_HOME_DUDE:
